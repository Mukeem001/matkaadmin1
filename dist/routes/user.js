...
function isValidBidNumber(gameType, number) {
    switch (gameType) {
        case "single_digit":
...
router.put("/user/profile", userAuthMiddleware, async (req, res) => {
...
// Markets Routes
router.get("/user/markets", userAuthMiddleware, async (req, res) => {
...
// Markets2 Routes
router.get("/user/markets2", userAuthMiddleware, async (req, res) => {
...
router.post("/user/bids", userAuthMiddleware, async (req, res) => {
    ...
    const [market] = await db.select().from(marketsTable).where(and(eq(marketsTable.id, marketId), eq(marketsTable.isActive, true)));
    if (!market) {
    ...
    const [existingBid] = await db.select()
        .from(bidsTable)
        .where(and(eq(bidsTable.userId, userId), eq(bidsTable.marketId, marketId), eq(bidsTable.gameType, gameType), eq(bidsTable.number, number)));
    if (existingBid) {
...
router.get("/user/bids", userAuthMiddleware, async (req, res) => {
...
router.post("/user/markets2-bids", userAuthMiddleware, async (req, res) => {
    const parsed = PlaceBidBody.safeParse(req.body);
    if (!parsed.success) {
        res.status(400).json({ error: "Invalid request", details: parsed.error.issues });
        return;
    ...
    const [market] = await db.select().from(markets2Table).where(and(eq(markets2Table.id, marketId), eq(markets2Table.isActive, true)));
    if (!market) {
        res.status(404).json({ error: "Market not found or market is inactive" });
        return;
    }
...
router.get("/user/markets2-bids", userAuthMiddleware, async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const offset = (page - 1) * limit;
    const userId = req.userId;
...
router.get("/user/markets2-results", userAuthMiddleware, async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const offset = (page - 1) * limit;
...
router.post("/user/deposits", userAuthMiddleware, async (req, res) => {
...
router.get("/user/deposits", userAuthMiddleware, async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const offset = (page - 1) * limit;
    const userId = req.userId;
...
router.post("/user/withdrawals", userAuthMiddleware, async (req, res) => {
...
router.get("/user/withdrawals", userAuthMiddleware, async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const offset = (page - 1) * limit;
    const userId = req.userId;
...
// Results Routes
router.get("/user/results", userAuthMiddleware, async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const offset = (page - 1) * limit;
...
// Dashboard Stats
router.get("/user/dashboard", userAuthMiddleware, async (req, res) => {
    const userId = req.userId;
    // Get user info
    const [user] = await db.select().from(usersTable).where(eq(usersTable.id, userId));
    // Get bid stats
    const [bidStats] = await db.select({
        totalBids: count(),
        totalAmount: sql `sum(cast(${bidsTable.amount} as decimal))`,
        wonBids: sql `count(case when ${bidsTable.status} = 'won' then 1 end)`,
        lostBids: sql `count(case when ${bidsTable.status} = 'lost' then 1 end)`,
        pendingBids: sql `count(case when ${bidsTable.status} = 'pending' then 1 end)`,
        totalWinnings: sql `sum(case when ${bidsTable.status} = 'won' then cast(${bidsTable.amount} as decimal) * 9 else 0 end)`, // Approximate winnings
    })
        .from(bidsTable)
        .where(eq(bidsTable.userId, userId));
    res.json({
...
// Win History
router.get("/user/wins", userAuthMiddleware, async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const offset = (page - 1) * limit;
    const userId = req.userId;
...
router.get("/user/leaderboard", userAuthMiddleware, async (req, res) => {
    ...
    const leaderboard = await db.select({
        ...
        totalWinnings: sql `sum(cast(${bidsTable.amount} as decimal))`,
        winCount: sql `count(*)`,
    })
        .from(bidsTable)
        .leftJoin(usersTable, eq(bidsTable.userId, usersTable.id))
        .where(and(eq(bidsTable.status, "won"), sql `${bidsTable.createdAt} >= ${today}`, sql `${bidsTable.createdAt} < ${tomorrow}`))
...
router.get("/user/notices", userAuthMiddleware, async (req, res) => {
...
router.get("/user/upi-methods", async (_req, res) => {
    try {
...
router.get("/user/apk-files", userAuthMiddleware, async (_req, res) => {
    try {
        ...
        res.status(500).json({ error: "Failed to fetch APK files", details: error.message });
    }
});
