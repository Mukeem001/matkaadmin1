...
const flexibleAuthMiddleware = (req, res, next) => {
    ...
    try {
        ...
        console.log("[Dashboard] ❌ Token verification failed:", err.message);
        res.status(401).json({ error: "Invalid token" });
    }
};
router.get("/dashboard/stats", flexibleAuthMiddleware, async (req, res) => {
    try {
        ...
        const depositsToday = parseFloat(depositsTodaySum[0]?.total ?? "0") || 0;
        const withdrawalsToday = parseFloat(withdrawalsTodaySum[0]?.total ?? "0") || 0;
        ...
        console.log("[Dashboard Stats] Stack:", err.stack);
