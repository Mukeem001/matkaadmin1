...
router.post("/settings/upload-qr", authMiddleware, qrUpload.single("qrCode"), async (req, res) => {
    try {
        if (!req.file) {
        ...
        res.status(500).json({ error: "Failed to upload QR code", details: error.message });
    }
});
