...
const router = Router();
// GET all APK files
router.get("/apk-files", authMiddleware, async (_req, res) => {
    try {
...
router.post("/apk-files", authMiddleware, upload.single("file"), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const size = typeof req.file.size === 'number' ? req.file.size : parseInt(req.file.size) || 0;
        ...
        res.status(500).json({ error: "Failed to upload APK file", details: error.message });
    }
});
// PUT update APK file metadata
router.put("/apk-files/:id", authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// DELETE APK file
router.delete("/apk-files/:id", authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const result = await db
