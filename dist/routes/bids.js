...
router.get("/bids", authMiddleware, async (req, res) => {
    try {
      ...
      LIMIT ${limit}
    `);
        const totalResult = await db.execute(sql `SELECT COUNT(*) as count FROM bids`);
 ...
 */
router.post("/process-pre-close/:marketId", authMiddleware, async (req, res) => {
    try {
 ...
 */
router.post("/process-now/:marketId", authMiddleware, async (req, res) => {
    try {
        const marketId = parseInt(req.params.marketId, 10);
        if (isNaN(marketId)) {
            res.status(400).json({ error: "Invalid market ID" });
