import { db, bids2Table, usersTable } from "@workspace/db";
import { eq, sql } from "drizzle-orm";
async function processMarket2Bids(marketId, result) {
    try {
        ...
        for (const bid of pendingBids) {
            ...
            if (isWinner) {
                ...
                console.log(`[Bid Processor 2] User ${bid.userId} WON ₹${winAmount} (${bid.betType}: ${bid.number} vs ${JSON.stringify(result)})`);
            }
            else {
                lostCount++;
                console.log(`[Bid Processor 2] User ${bid.userId} LOST (${bid.betType}: ${bid.number} vs ${JSON.stringify(result)})`);
            }
        }
        return {
 ...
 */
function checkBid2Winner(betType, bidNumber, result) {
    switch (betType) {
        case "left_digit":
 ...
 */
function calculateWinnings2(bidAmount, betType) {
    let multiplier = 1;
    switch (betType) {
        case "left_digit":
        case "right_digit":
...
//# sourceMappingURL=bid-processor2.js.map