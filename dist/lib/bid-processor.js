 ...
 */
export function isBidWinner(bidNumber, gameType, result) {
 ...
 */
export function calculateWinnings(bidAmount, gameType, rates) {
 ...
 */
export async function processMarketBids(marketId, result) {
    ...
    const gameRates = {
        singleDigit: parseFloat(rates.singleDigit),
        jodiDigit: parseFloat(rates.jodiDigit),
        singlePanna: parseFloat(rates.singlePanna),
        doublePanna: parseFloat(rates.doublePanna),
        triplePanna: parseFloat(rates.triplePanna),
        halfSangam: parseFloat(rates.halfSangam),
        fullSangam: parseFloat(rates.fullSangam),
    };
 ...
 */
export async function processMarketBidsPreClose(marketId) {
    try {
        // Get market details
        ...
        const [result] = await db.select().from(resultsTable).where(and(eq(resultsTable.marketId, marketId), eq(resultsTable.resultDate, today)));
