...
function isAfterCloseWindow(closeTime) {
...
// ================= SCRAPER =================
export async function scrapeResult(url, marketName) {
...
async function scrapeSattaMatkaComIn(marketName) {
...
export async function scrapeLiveResults(marketName) {
    try {
        console.log("\n========== 🔴 [LIVE] SCRAPING START ==========");
...
// ================= MAIN FUNCTION =================
export async function fetchAndUpdateMarketResult(marketId) {
    ...
    const isValid = scraped.openResult &&
        scraped.closeResult &&
        scraped.jodiResult;
    if (!isValid) {
    ...
    const [existingResult] = await db
        .select()
        .from(resultsTable)
        .where(and(eq(resultsTable.marketId, marketId), eq(resultsTable.resultDate, resultDateStr)));
    if (existingResult) {
