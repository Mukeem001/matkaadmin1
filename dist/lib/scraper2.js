 ...
 */
async function fetchAndUpdateMarkets2Result(marketId) {
    try {
 ...
 */
async function scrapeMarkets2Result(url, marketName) {
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (cols.length >= 3) {
                ...
                if (name.includes(target)) {
                    ...
                    // Priority → Today
                    if (isValidResult(today))
                        return today;
                    // Fallback → Previous
                    if (isValidResult(prev))
                        return prev;
                }
            }
        }
